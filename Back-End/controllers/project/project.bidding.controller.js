...
export const submitBid = async (req, res) => {
  try {
    ...
    await createNotification(
      ...
      'bid',
      'New Bid Received',
      `${user.fullname} submitted a bid of $${amount} on your project "${project.title}"`,
      { projectId: id, bidId: project.bids[project.bids.length - 1]._id, amount }
    );

    // Emit real-time update to project room
    emitToProject(id, 'new-bid', {
      projectId: id,
...
export const acceptBid = async (req, res) => {
  try {
    ...
    await createNotification(
      ...
      { projectId: project._id, bidId: bid._id }
    );

    // Emit socket event
    emitToProject(id, 'bid-accepted', {
      bidId: bid._id,
