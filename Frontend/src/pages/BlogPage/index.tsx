...
export default function BlogPage({ onNavigate }: BlogPageProps) {
  ...
  return (
    <div className="bg-gradient-to-br from-[#0A0E27] via-[#1a1f3a] to-[#0f1629] min-h-screen">
      ...
      <section className="py-8 px-6 lg:px-12 border-b border-white/10">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {categories.map((category) => {
              ...
              return (
                <button
                  key={category.id}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:border-cyan-500 hover:bg-white/10 transition-all font-medium text-gray-300 hover:text-cyan-400"
                >
                  <Icon size={18} />
                  {category.name}
                </button>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featuredBlogs.map((blog) => (
              <article
                key={blog.id}
                className="bg-white/5 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-2xl hover:shadow-cyan-500/20 transition-all duration-300 overflow-hidden border border-white/10 hover:border-cyan-500/50 group cursor-pointer"
              >
