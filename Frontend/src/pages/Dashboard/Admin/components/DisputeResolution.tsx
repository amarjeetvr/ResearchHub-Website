...
import { Search, Filter, RefreshCw, Download, Eye, AlertTriangle, DollarSign, Calendar, User, Briefcase, Clock, CheckCircle, XCircle, Grid, List, TrendingUp } from 'lucide-react';
...
export default function DisputeResolution({ disputes }: { disputes: Dispute[] }) {
  ...
  const enhancedDisputes = disputes.map(d => ({
    ...d,
    priority: d.priority || (Math.random() > 0.7 ? 'high' : Math.random() > 0.5 ? 'medium' : 'low'),
    description: d.description || 'Payment dispute regarding project completion and deliverables.'
  }));
  ...
  const handleExport = () => {
    const csvContent = "data:text/csv;charset=utf-8," + 
      "ID,Project,Client,Freelancer,Amount,Status,Priority,Date\n" +
      filteredDisputes.map(d => 
        `${d.id},"${d.project}","${d.client}","${d.freelancer}",${d.amount},"${d.status}","${d.priority}","${d.date}"`
      ).join("\n");
  ...
  const getStatusBadge = (status: string) => {
    const config = {
      open: { bg: 'bg-gradient-to-r from-red-100 to-rose-100', text: 'text-red-700', border: 'border-red-200', icon: AlertTriangle },
      'in-review': { bg: 'bg-gradient-to-r from-blue-100 to-indigo-100', text: 'text-blue-700', border: 'border-blue-200', icon: Eye },
      resolved: { bg: 'bg-gradient-to-r from-green-100 to-emerald-100', text: 'text-green-700', border: 'border-green-200', icon: CheckCircle },
      closed: { bg: 'bg-gradient-to-r from-gray-100 to-slate-100', text: 'text-gray-700', border: 'border-gray-200', icon: XCircle }
    };
    const { bg, text, border, icon: Icon } = config[status as keyof typeof config] || config.open;
    return (
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="p-6 border-b border-slate-200/50 bg-gradient-to-r from-slate-50/50 to-red-50/50">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600">Resolve conflicts between clients and freelancers</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <motion.button 
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-red-100 text-red-700 rounded-xl font-semibold hover:bg-red-200 transition-all"
            >
              <Download size={16} />
              Export
            </motion.button>
        ...
        <div className="flex flex-col lg:flex-row gap-4 mt-6">
          <div className="flex flex-col sm:flex-row gap-3 flex-1">
            <select
              ...
              className="px-4 py-2.5 bg-white/80 backdrop-blur-sm border border-slate-300/50 text-slate-900 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-red-500 transition-all shadow-sm"
            >
            ...
            <div className="relative flex-1 min-w-[300px]">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white/80 backdrop-blur-sm border border-slate-300/50 text-slate-900 placeholder-slate-500 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-red-500 transition-all shadow-sm"
              />
            </div>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50/50">
                <tr className="text-left">
                  ...
                  <th className="px-6 py-4 font-semibold text-slate-700 text-sm">Parties</th>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-gradient-to-br from-red-100 to-rose-100 rounded-xl">
                          <Briefcase className="text-red-600" size={16} />
                        </div>
                        <div>
                          <div className="font-semibold text-slate-900">{dispute.project}</div>
                          <div className="text-xs text-slate-500">ID: {dispute.id}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-slate-900">{dispute.client}</span>
                          <span className="text-xs text-slate-500">(Client)</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <User size={12} className="text-green-500" />
                          <span className="text-sm font-medium text-slate-900">{dispute.freelancer}</span>
                          <span className="text-xs text-slate-500">(Freelancer)</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-green-600">${dispute.amount.toLocaleString()}</span>
                      </div>
                      <div className="text-xs text-slate-500">In Escrow</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-slate-600">
                        <Calendar size={14} />
                        <span className="text-sm">{dispute.date}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                ...
                >
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-bold text-slate-900 mb-1">{dispute.project}</h3>
                  ...
                  <div className="space-y-3 mb-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium">{dispute.client}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-slate-600">Freelancer:</span>
                      <span className="text-sm font-medium">{dispute.freelancer}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-slate-600">Amount:</span>
                      <span className="text-sm font-bold text-green-600">${dispute.amount.toLocaleString()}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-slate-600">Reported:</span>
                      <span className="text-sm font-medium">{dispute.date}</span>
                    </div>
                  </div>
        ...
        <div className="px-6 py-4 border-t border-slate-200/50 bg-slate-50/30">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <span className="text-sm text-slate-600">
              Showing {startIndex + 1}-{Math.min(endIndex, filteredDisputes.length)} of {filteredDisputes.length} disputes
            </span>
            <div className="flex items-center gap-2">
              <button 
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-red-100 to-rose-100 border border-red-200 rounded-2xl p-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="text-sm text-red-700 font-medium">Open</div>
            </div>
            <div className="text-2xl font-bold text-red-800">
              {enhancedDisputes.filter(d => d.status === 'open').length}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-100 to-indigo-100 border border-blue-200 rounded-2xl p-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="text-sm text-blue-700 font-medium">In Review</div>
            </div>
            <div className="text-2xl font-bold text-blue-800">
              {enhancedDisputes.filter(d => d.status === 'in-review').length}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-green-100 to-emerald-100 border border-green-200 rounded-2xl p-4">
            ...
            <div className="text-2xl font-bold text-green-800">
              {enhancedDisputes.filter(d => d.status === 'resolved').length}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-purple-100 to-violet-100 border border-purple-200 rounded-2xl p-4">
            ...
            <div className="text-2xl font-bold text-purple-800">
              ${enhancedDisputes.reduce((sum, d) => sum + d.amount, 0).toLocaleString()}
            </div>
          </div>
        </div>
      </div>
